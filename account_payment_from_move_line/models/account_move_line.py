...
def _check_has_partner(line: "account.move.line", context: dict):
    if not line.partner_id:
        raise UserError(_("The journal item has no partner."))
...
def _check_is_posted(line: "account.move.line", context: dict):
    if line.move_id.state != "posted":
        raise UserError(
            _("Only posted journal items " "can be selected to register a payment.")
        )


def _check_is_not_reconciled(line: "account.move.line", context: dict):
    if line.full_reconcile_id:
        raise UserError(_("The journal item is already reconciled."))
...
def _check_lines_have_common_commercial_partner(
    lines: "account.move.line", context: dict
):
...
def _check_at_least_one_line_selected(lines: "account.move.line", context: dict):
    if not lines:
        raise UserError(
            _("You must select at least one journal item to generate payments.")
        )
