...
class MisBuilderXlsx(models.AbstractModel):
    ...
    def _add_filter_row(self, objects, sheet, row_pos):
    ...
    def _create_sheet(self, workbook, objects, matrix, report_name, sheet):
        ...
        # filters
        row_pos = self._add_filter_row(objects, sheet, row_pos)
        ...
        for row in matrix.iter_rows():
            if (
                ...
                continue
            row_xlsx_style = \
                style_obj.to_xlsx_style(TYPE_STR, row.style_props)
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        workbook.file_name = objects.display_name
        sheet = workbook.add_worksheet(report_name[:31])
        self._create_sheet(
            workbook, objects, matrix, report_name, sheet
        )
        if objects._context.get('automated_edition'):
            if objects.analytic_account_iteration:
                for analytic_account_id in \
                        self.env["account.analytic.account"].search([]):
                    context = {"analytic_account_id": {
                        "value": analytic_account_id.id
                    }}
                    matrix = objects.with_context(mis_report_filters=context
                                                  )._compute_matrix()

                    sheet = workbook.add_worksheet(analytic_account_id.name)
                    self._create_sheet(
                        workbook, objects, matrix, report_name, sheet
                    )
            if objects.analytic_group_account_iteration:
                for analytic_account_group_id in \
                        self.env["account.analytic.group"].search([]):
                    context = {"analytic_account_id.group_id": {
                        "value": analytic_account_group_id.id
                    }}
                    matrix = objects.with_context(mis_report_filters=context
                                                  )._compute_matrix()

                    sheet = workbook.add_worksheet(
                        analytic_account_group_id.name)
                    self._create_sheet(
                        workbook, objects, matrix, report_name, sheet
                    )
