...
class TestBankStatementPartnerMappingBase(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        # Prepare setting
        cls.acc_stmt_partner_mapping_model.with_user(cls.manager_account_id).create(
            {
                "label": "RET Source",
        ...
        cls.bank_stmt_line = cls.acc_bank_stmt_line_model.with_user(
            cls.user_account_id
        ).create(
            {
                "payment_ref": "Compte A Payer",
        ...
        cls.bank_stmt_partner_line = cls.acc_bank_stmt_line_model.with_user(
            cls.user_account_id
        ).create(
            {
                "payment_ref": "Compte A Payer",
        ...
        cls.bank_stmt_patial_partner_line = cls.acc_bank_stmt_line_model.with_user(
            cls.user_account_id
        ).create(
            {
                "payment_ref": "RET Source 20",
                "statement_id": cls.bank_stmt.id,
                "amount": 500,
                "date": time.strftime("%Y") + "-01-01",
            }
        ...
        cls.statement_line = cls.acc_bank_stmt_line_model.with_user(
            cls.user_account_id
        ).create(
            {
                "payment_ref": "Compte A Payer",
