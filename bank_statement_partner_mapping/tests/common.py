...
class TestBankStatementPartnerMappingBase(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_account = mail_new_test_user(cls.env, login='sebastien', groups='account.group_account_user')
        cls.user_account_id = cls.user_account.id
        cls.manager_account = mail_new_test_user(cls.env, login='frederic', groups='account.group_account_manager')
        ...
        cls.partner_stmt_partial = cls.env['res.partner'].create({'name': 'Revenu Quebec'})
        cls.partner_stmt = cls.env['res.partner'].create({'name': 'Deco'})

        # Prepare setting
        cls.acc_stmt_partner_mapping_model.sudo(cls.manager_account_id).create({'label': 'RET Source',
                                                                                  'mapping_type': 'partial',
                                                                                  'partner_id': cls.partner_stmt_partial.id})
        cls.acc_stmt_partner_mapping_model.sudo(cls.manager_account_id).create({'label': 'Compte A Payer',
        ...
        cls.bank_stmt_line = cls.acc_bank_stmt_line_model.sudo(cls.user_account_id).create({'name': 'Compte A Payer',
                                                                                               'statement_id': cls.bank_stmt.id,
                                                                                               'amount': 40,
                                                                                               'date': time.strftime(
                                                                                                   '%Y') + '-01-01', })
        # create statement line with partner and without journal entry
        cls.bank_stmt_partner_line = cls.acc_bank_stmt_line_model.sudo(cls.user_account_id).create(
            {'name': 'Compte A Payer',
             'statement_id': cls.bank_stmt.id,
             'amount': 500,
        ...
        cls.bank_stmt_patial_partner_line = cls.acc_bank_stmt_line_model.sudo(cls.user_account_id).create(
            {'name': 'RET Source 20',
             'statement_id': cls.bank_stmt.id,
             'amount': 500,
             'date': time.strftime('%Y') + '-01-01', })
        ...
        cls.statement = cls.acc_bank_stmt_model.create({
            'journal_id': cls.bank_journal_euro.id,
            'balance_end_real': 500})

        cls.statement_line = cls.acc_bank_stmt_line_model.sudo(cls.user_account_id).create(
            {'name': 'Compte A Payer',
