...
def _apply_mapping(value, mapping):
    """Apply all matching term replacements to a string."""
    for old_term, new_term in mapping.items():
        if old_term in value:
...
class CodeTranslations(BaseCodeTranslations):
    def get_python_translations(self, module_name, lang):
        # In v18, get_python_translations caches results as ReadonlyDict.
        # We intercept on first load to apply the CA term mapping.
    ...
    def get_web_translations(self, module_name, lang):
        # In v18, web_translations are stored as:
        #   ReadonlyDict({'messages': tuple(ReadonlyDict({'id': src, 'string': value}) ...)})
        # We intercept on first load to apply the CA term mapping.
