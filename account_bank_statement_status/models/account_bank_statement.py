...
class AccountBankStatement(models.Model):
    ...
    is_reconciled = fields.Boolean(
        string="Approved", compute="_compute_is_reconciled", store=True
    )
    ...
    def _compute_is_reconciled(self):
        for stmt in self:
            stmt.is_reconciled = (
                all(line.is_reconciled for line in stmt.line_ids) and stmt.is_complete
            )
    ...
    def action_confirm_statement(self):
        for rec in self:
            ...
            if not rec.is_complete:
                raise ValidationError(
                    _(
                        "Sum of statement lines must equal to the "
