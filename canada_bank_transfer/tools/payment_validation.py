...
def check_payment_method_is_eft(payments, context):
    ...
    payments_with_wrong_method = payments.filtered(
        lambda p: p.payment_method_id.code != "eft"
    )
...
def check_bank_is_selected_on_bank_accounts(payments, context):
    accounts_with_no_bank = payments.mapped("partner_bank_id").filtered(
        lambda a: not a.bank_id
    )
...
def check_transit_number_is_set_on_bank_accounts(payments, context):
    accounts_with_no_transit = payments.mapped("partner_bank_id").filtered(
        lambda a: not a.canada_transit
    )
...
def check_institution_number_is_set_on_banks(payments, context):
    banks_with_no_institution = payments.mapped("partner_bank_id.bank_id").filtered(
        lambda b: not b.canada_institution
    )
...
def check_account_number_between_7_and_12_digits(payments, context):
    accounts_with_wrong_chars = payments.mapped("partner_bank_id").filtered(
        lambda a: not (a.acc_number or "").isdigit()
    )
    ...
    accounts_with_more_12_digits = payments.mapped("partner_bank_id").filtered(
        lambda a: len(a.acc_number or "") > 12
    )
    ...
    accounts_with_less_7_digits = payments.mapped("partner_bank_id").filtered(
        lambda a: len(a.acc_number or "") < 7
    )
