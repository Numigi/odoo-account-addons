...
class TestAccountMoveUniqueReversal(common.TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls.account_1 = cls.env["account.account"].create(
            {
                "name": "Account 1",
                "code": "501001",
                "account_type": "expense",  # Remplacement V18
            }
        )
        cls.account_2 = cls.env["account.account"].create(
            {
                "name": "Account 2",
                "code": "101001",
                "account_type": "asset_fixed",  # Remplacement V18
            }
        )
        ...
        move = cls.env["account.move"].create(
            {
                ...
                "date": cls.today,
                "line_ids": [
                    (0, 0, {"account_id": cls.account_1.id, "name": "/",
                            "debit": 75}),
                    (0, 0, {"account_id": cls.account_1.id, "name": "/",
                            "debit": 25}),
                    (0, 0, {"account_id": cls.account_2.id, "name": "/",
                            "credit": 100},),
                ],
            })
        return move
    ...
    def test_reverse_reversal_entry_fail(self):
        self._reverse_move(self.move)
        # Odoo 18: on récupère le premier élément de la liste reversal_move_ids
        reversal_move = self.move.reversal_move_ids[0]
        with self.assertRaises(UserError):
            self._reverse_move(reversal_move)

    def test_reverse_reversal_entry_sale_pass(self):
        self._reverse_move(self.move_sale)
        reversal_move = self.move_sale.reversal_move_ids[0]
        assert self._reverse_move(reversal_move)

    def test_reverse_reversal_entry_purchase_pass(self):
        self._reverse_move(self.move_purchase)
        reversal_move = self.move_purchase.reversal_move_ids[0]
        assert self._reverse_move(reversal_move)

    def _reverse_move(self, move):
        wizard_env = self.env["account.move.reversal"].with_context(
