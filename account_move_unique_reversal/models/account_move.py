...
class AccountMove(models.Model):
    ...
    def _reverse_moves(self, default_values_list=None, cancel=False):
        for move in self:
            if move.reversal_move_ids and move.journal_id.type not in (
                "sale",
                "purchase",
            ):
                reversals_name = ", ".join(move.reversal_move_ids.mapped('display_name'))
                raise UserError(
                    _(
                        "The accounting entry {} is already reversed (by entry {}). "
                        "You can only reverse an accounting entry once."
                    ).format(move.display_name, reversals_name)
                )
